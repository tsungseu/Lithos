...
(async()=>{
 ...
 try{
  ...
  await page.locator('.obs-files input[type=checkbox]').first().check();
  const selected=await page.locator('#selection-count').innerText();await shot('project');
  await page.locator('#project-ai-distill').click();
  ...
  await page.getByRole('button',{name:'关闭设置'}).click();await command('help');await shot('help');
