...
async function selectGraphNode(n){
 ...
 if(n.kind==='directory'){
  ...
  const open=node('button','打开为文档标签','secondary');open.onclick=()=>window.LithosWorkspace?.openDocument(n.id,true)||window.LithosKnowledge?.open(n.id,true);$('graph-actions').append(open);
 }
